...
export async function PlayerDevelopmentSection({ playerId, development }: { playerId: string; development: PlayerDevelopmentProfile }) {
    ...
    <section className="rounded-lg border border-board-line bg-white p-5 shadow-soft">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">{ui("Connect observations to long-term player development goals.")}</p>
        </div>
        <ButtonLink href="/squad/development" variant="secondary" className="h-9 px-3">{ui("Development overview")}</ButtonLink>
      </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="space-y-4">
          <details className="rounded-md border border-board-line bg-board-paper p-4">
            <summary className="cursor-pointer text-sm font-bold text-board-navy">{ui("Add development goal")}</summary>
          ...
          {development.goals.length ? (
            ...
            <div className="rounded-md border border-dashed border-board-line p-5">
              <p className="font-bold text-board-navy">{ui("No development goals yet.")}</p>
              <p className="mt-1 text-sm text-slate-600">{ui("Add one observable focus area, then record progress and evidence over time.")}</p>
            </div>
          )}
        </div>
        ...
        <aside className="space-y-4">
          <div className="rounded-md bg-slate-50 p-4">
            <p className="text-sm font-bold text-board-navy">{ui("Quick observation")}</p>
            {activeGoals.length ? <p className="mt-1 text-xs text-slate-500">{ui("Active goals: ")}{activeGoals.map((goal) => goal.title).join(", ")}</p> : null}
...
export async function ObservationForm({
  playerId,
  goals,
  eventId,
  returnTo,
  ...
  return (
    <form action={createPlayerObservation} className={compact ? "mt-3 space-y-2" : "space-y-3"}>
      ...
      <label className="block">
        <span className="text-xs font-bold uppercase text-slate-500">{ui("Observation")}</span>
        <textarea name="note" required rows={compact ? 2 : 3} placeholder={ui("What did you notice?")} className="mt-1 w-full rounded-md border border-board-line bg-white px-3 py-2 text-sm outline-none focus:border-board-green focus:ring-4 focus:ring-green-100" />
      </label>
      <Button type="submit" variant="secondary" className="h-10 px-3">
        <Plus className="h-4 w-4" />
        {ui("Add observation")}</Button>
    </form>
  );
}

async function DevelopmentGoalForm({ playerId }: { playerId: string }) {
  const locale = await getActiveLocale();
  const ui = createSystemTranslator(locale);
  return (
    <form action={createDevelopmentGoal} className="mt-4 grid gap-3 md:grid-cols-2">
      ...
      <label className="md:col-span-2">
        <span className="text-xs font-bold uppercase text-slate-500">{ui("Goal title")}</span>
        <input name="title" required maxLength={120} placeholder={ui("Scanning before receiving")} className="mt-1 h-10 w-full rounded-md border border-board-line bg-white px-3 text-sm outline-none focus:border-board-green focus:ring-4 focus:ring-green-100" />
      </label>
      <label className="md:col-span-2">
        <span className="text-xs font-bold uppercase text-slate-500">{ui("Success criteria")}</span>
        <textarea name="successCriteria" required rows={2} placeholder={ui("What improved behaviour should be visible?")} className="mt-1 w-full rounded-md border border-board-line bg-white px-3 py-2 text-sm outline-none focus:border-board-green focus:ring-4 focus:ring-green-100" />
      </label>
      <label className="md:col-span-2">
        <span className="text-xs font-bold uppercase text-slate-500">{ui("Coach notes")}</span>
        <textarea name="coachNotes" rows={2} placeholder={ui("Why was this goal created?")} className="mt-1 w-full rounded-md border border-board-line bg-white px-3 py-2 text-sm outline-none focus:border-board-green focus:ring-4 focus:ring-green-100" />
      </label>
      <SelectField name="category" label={ui("Category")} options={developmentGoalCategories} defaultValue="technical" />
      <SelectField name="priority" label={ui("Priority")} options={developmentGoalPriorities} defaultValue="medium" />
      <SelectField name="status" label={ui("Status")} options={developmentGoalStatuses} defaultValue="in_progress" />
      <SelectField name="progress" label={ui("Latest progress")} options={developmentProgressOptions} defaultValue="developing" />
      <label>
        <span className="text-xs font-bold uppercase text-slate-500">{ui("Start date")}</span>
      ...
      <label>
        <span className="text-xs font-bold uppercase text-slate-500">{ui("Target review date")}</span>
      ...
      <div className="md:col-span-2">
        <Button type="submit">{ui("Create goal")}</Button>
      </div>
    </form>
  );
}

async function DevelopmentGoalCard({ goal, playerId }: { goal: PlayerDevelopmentGoal; playerId: string }) {
  const locale = await getActiveLocale();
  const ui = createSystemTranslator(locale);
  ...
  return (
    <article className="rounded-md border border-board-line bg-white p-4">
      ...
      <details className="mt-4 rounded-md bg-slate-50 p-3">
        <summary className="cursor-pointer text-sm font-bold text-board-navy">{ui("Edit goal")}</summary>
        <form action={updateDevelopmentGoal} className="mt-3 grid gap-2 lg:grid-cols-3">
          <input type="hidden" name="goalId" value={goal.id} />
          <label className="lg:col-span-3">
            <span className="text-xs font-bold uppercase text-slate-500">{ui("Title")}</span>
          ...
          <SelectField name="category" label={ui("Category")} options={developmentGoalCategories} defaultValue={goal.category} compact />
          <SelectField name="priority" label={ui("Priority")} options={developmentGoalPriorities} defaultValue={goal.priority} compact />
          <SelectField name="status" label={ui("Status")} options={developmentGoalStatuses} defaultValue={goal.status} compact />
          <SelectField name="progress" label={ui("Latest progress")} options={developmentProgressOptions} defaultValue={goal.progress} compact />
          <label>
            <span className="text-xs font-bold uppercase text-slate-500">{ui("Target review")}</span>
          ...
          <div className="lg:col-span-3">
            <Button type="submit" variant="secondary" className="h-10 px-3">{ui("Update goal")}</Button>
          </div>
        </form>
      </details>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-3">
        <div>
          <p className="flex items-center gap-2 text-sm font-bold text-board-navy"><Target className="h-4 w-4" />{ui("Progress history")}</p>
          ...
          <form action={createDevelopmentProgressUpdate} className="mt-3 grid gap-2 rounded-md bg-board-paper p-3">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <SelectField name="progressLevel" label={ui("Progress")} options={developmentProgressOptions} defaultValue={goal.progress} compact />
              <label>
                <span className="text-xs font-bold uppercase text-slate-500">{ui("Date")}</span>
            ...
            <textarea name="note" required rows={2} placeholder={ui("What changed since the last review?")} className="w-full rounded-md border border-board-line bg-white px-3 py-2 text-sm outline-none focus:border-board-green focus:ring-4 focus:ring-green-100" />
            <Button type="submit" variant="secondary" className="h-10 px-3">{ui("Add progress update")}</Button>
          </form>
        </div>

        <div>
          <p className="flex items-center gap-2 text-sm font-bold text-board-navy"><ClipboardCheck className="h-4 w-4" />{ui("Coach actions")}</p>
          <div className="mt-2 space-y-2">
            ...
            )) : <p className="rounded-md border border-dashed border-board-line p-3 text-sm text-slate-500">{ui("No actions yet.")}</p>}
          </div>
          <form action={createGoalAction} className="mt-3 grid gap-2">
            <input type="hidden" name="goalId" value={goal.id} />
            <input name="description" required placeholder={ui("Add a coaching action")} className="h-10 rounded-md border border-board-line bg-white px-3 text-sm outline-none focus:border-board-green focus:ring-4 focus:ring-green-100" />
            <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
              ...
              <Button type="submit" variant="secondary" className="h-10 px-3">{ui("Add action")}</Button>
            </div>
          </form>
        </div>

        <div>
          <p className="flex items-center gap-2 text-sm font-bold text-board-navy"><Eye className="h-4 w-4" />{ui("Linked observations")}</p>
          <div className="mt-2 space-y-2">
          ...
          <details className="mt-3 rounded-md bg-board-paper p-3">
            <summary className="cursor-pointer text-sm font-bold text-board-navy">{ui("Add observation for this goal")}</summary>
...
async function DevelopmentTimeline({ development }: { development: PlayerDevelopmentProfile }) {
  const locale = await getActiveLocale();
  const ui = createSystemTranslator(locale);
  return (
    <div className="rounded-md border border-board-line p-4">
      <p className="flex items-center gap-2 text-sm font-bold text-board-navy"><CalendarDays className="h-4 w-4" />{ui("Development timeline")}</p>
      <div className="mt-3 space-y-3">
        {development.timeline.length ? development.timeline.slice(0, 8).map((item) => (
          ...
          <p className="rounded-md border border-dashed border-board-line p-3 text-sm text-slate-500">{ui("No development timeline yet.")}</p>
        )}
      </div>
    </div>
  );
